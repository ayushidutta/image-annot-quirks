function [X, fX, i] = minimize(X, f, length, verb, varargin)
...
% absolute gives the maximum allowed number of function evaluations. If "length"
% has a decimal value, it is used as a threshold on the relative change of the
% function evaluations . You can
...
rel_df_thr = abs(length(1)) - floor(abs(length(1)));
if max(size(length)) == 2, red=length(2); length=length(1); else red=1; end
if length>0, S='Linesearch'; else S='Function evaluation'; end

i = 0;                                            % zero the run length counter
rel_df = 1;                  % set initial relative variation of function value
...
while (i < abs(length)) && (rel_df > rel_df_thr)           % while not finished
  i = i + (length>0);                                      % count iterations?!
  rel_df = f0;

  X0 = X; F0 = f0; dF0 = df0;                   % make a copy of current values
  if length>=1, M = MAX; else M = min(MAX, -length-i); end
  ...
  if abs(d3) < -SIG*d0 && f3 < f0+x3*RHO*d0          % if line search succeeded
    ...
    if verb, fprintf('%s %6i;  Value %4.6e;  Rel.Change %4.6e\r', S, i, f0,abs(rel_df - f0)/(f0+eps) ); end
